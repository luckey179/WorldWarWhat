...
static var sensitivity : float = 360.0;
static var sensitivity1 : float = 200.0;
...
function RotateUpDown(axis : float){
	transform.RotateAround(transform.position , transform.right, -axis * Time.unscaledDeltaTime * sensitivity1);

}

//rotate the camera rigt and left (y rotation)
function RotateRightLeft(axis : float){
 	transform.RotateAround(transform.position, Vector3.up, -axis * Time.unscaledDeltaTime * sensitivity);
